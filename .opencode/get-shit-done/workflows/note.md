...
- **Global scope**: `C:/dev/INDT/InsuranceManager/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/dev/INDT/InsuranceManager/.opencode/notes/`)
...
2. Glob `C:/dev/INDT/InsuranceManager/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (C:/dev/INDT/InsuranceManager/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/dev/INDT/InsuranceManager/.opencode/notes/` — works in any directory
